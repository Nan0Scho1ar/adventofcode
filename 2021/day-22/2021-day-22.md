...
[cuboid](https://en.wikipedia.org/wiki/Cuboid){target="_blank"} (the set
of all cubes that have coordinates which fall within ranges for `x`,
`y`, and `z`) and whether to turn all of the cubes in that cuboid *on*
or *off*.
