...
displays](https://en.wikipedia.org/wiki/Seven-segment_display){target="_blank"}
in your submarine are malfunctioning; [they must have been
...
(the part after `|` on each line), in the above example, there are
*`26`* instances of digits that use a unique number of segments
(highlighted above).
