...
starts leaking from the submarine\'s [arithmetic logic
...
-   `mod a b` - Divide the value of `a` by the value of `b`, then store
    ...
    [modulo](https://en.wikipedia.org/wiki/Modulo_operation){target="_blank"}
    operation.)
...
[tanuki](https://en.wikipedia.org/wiki/Japanese_raccoon_dog){target="_blank"}.
