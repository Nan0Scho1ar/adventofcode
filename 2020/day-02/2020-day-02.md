...
[toboggan](https://en.wikipedia.org/wiki/Toboggan){target="_blank"}.
...
-   `1-3 a: `*`a`*`b`*`c`*`de` is *valid*: position `1` contains `a` and
    position `3` does not.
-   `1-3 b: `*`c`*`d`*`e`*`fg` is *invalid*: neither position `1` nor
    position `3` contains `b`.
-   `2-9 c: c`*`c`*`cccccc`*`c`* is *invalid*: both position `2` and
    position `9` contain `c`.
